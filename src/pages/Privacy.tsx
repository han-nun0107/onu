...
export default function Privacy() {
  return (
    <div className="mx-auto max-w-[700px] px-4 py-6 font-['Noto_Sans_KR'] leading-[1.7] sm:px-6 sm:py-8 md:px-8 md:py-12">
      ...
      {PRIVACY_SECTIONS.map((section) => (
        <div key={section.id}>
          <h2 className="mt-6 text-lg font-semibold text-gray-700 sm:mt-8 sm:text-xl">
            {section.id}. {section.title}
          </h2>
          {section.type === "list" && Array.isArray(section.content) && (
            <ul className="mb-4 ml-4 list-disc space-y-2 text-sm sm:mb-6 sm:ml-6 sm:text-base">
              {section.content.map((item) => (
                <li key={`${section.id}-${item.substring(0, 30)}`}>{item}</li>
              ))}
            </ul>
          )}
